...
public class PosPrinterService {
        ...
        try {
            ...
            if (orderVO.getPayments() != null) {
                for (OrderDetailVO.OrderPayVO pay : orderVO.getPayments()) {
                    // 🌟 铁闸隔离：即使有数据，非会员也绝不打印会员支付
                    if ("BALANCE".equals(pay.getPayMethodCode()) && isVip) {
                        writeText(bos, "会员支付: " + fmt(pay.getPayAmount()) + "\n");
                    } else if ("CASH".equals(pay.getPayMethodCode())) {
                        writeText(bos, "现金支付: " + fmt(pay.getPayAmount()) + "\n");
                    } else if ("AGGREGATE".equals(pay.getPayMethodCode())) {
                        writeText(bos, getPayName(pay.getPayTag()) + ": " + fmt(pay.getPayAmount()) + "\n");
                    }
                }
            }

            // 🌟 铁闸隔离：所有优惠券/满减/会员信息，严格限制在 isVip 为 true 时打印
            if (isVip) {
                ...
                if (orderVO.getUseVoucherAmount() != null && orderVO.getUseVoucherAmount().compareTo(BigDecimal.ZERO) > 0) {
                    writeText(bos, "满减抵扣: " + fmt(orderVO.getUseVoucherAmount()) + " (1张)\n");
                }
            ...
            if (StringUtils.hasText(config.getShopPhone())) {
                writeText(bos, "联系电话: " + config.getShopPhone() + "\n");
            }
    ...
    private String getPayName(String tag) {
        if (!StringUtils.hasText(tag)) return "扫码支付";
        if (tag.toUpperCase().contains("WX") || tag.contains("微信")) return "微信支付";
        if (tag.toUpperCase().contains("ALI") || tag.contains("支付宝")) return "支付宝支付";
        if (tag.endsWith("支付")) return tag;
        return tag + "支付";
    }
